Shader "Custom/ColorInverter" {
	Properties {
		...
		_EmissiveColor ("Emissive", Color) = (0,0,0,1)
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			half _NoisyInverseAmount = _InvertAmount + (tex2D(_Noise, IN.uv_Noise + _Time/20 +.5).r + tex2D(_Noise, IN.uv_Noise -_Time/20).r)/2;

			if (_NoisyInverseAmount <= 0) {
				_NoisyInverseAmount = -1;
				}
			else if (_NoisyInverseAmount > 0) {
				_NoisyInverseAmount = 1;
			}
